...
$(() => {
  ...
  $('#num-guests-dec').on('click', () => {
    const currentNumGuest = Number($('#num-guests').val());
    $('#num-guests').val(Math.max(currentNumGuest - 1, 1));
  });
  $('#num-guests-inc').on('click', () => {
    const currentNumGuest = Number($('#num-guests').val());
  ...
  function fillCalendar(currentMonthIndex) {
    ...
    const calendar = makeCalendar(currentMonthIndex);
  ...
  function fillTimeSlots(year, month, day) {
    ...
    for (const slot of timeslots) {
      ...
      const className = slot.occupied ? 'occupied' : '';
   ...
   * ВЫБОР МЕСЯЦА
   *************************************************/
  $('#calendar .month-selector button').on('click', (event) => {
   ...
   * ВЫБОР ДНЯ
   *************************************************/
  $('#calendar .day-selector').on('click', 'td', (event) => {
   ...
   * ВЫБОР ВРЕМЕНИ
   *************************************************/
  $('#calendar .time-selector').on('click', 'button', (event) => {
    const button = $(event.target);
